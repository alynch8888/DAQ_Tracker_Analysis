...
def main():
    ...
    #Adds in a new definition for evt_time.
    df = df.Define(
        "evt_time",
        "sh.time.empty() ? std::numeric_limits<float>::quiet_NaN() : Min(sh.time)"
    )

    data_time = df.AsNumpy(["srn", "evt_time"])
    data_hits = df.AsNumpy(["sh.mnid","srn"])
    pdf = pd.DataFrame(data_time)
    ...
    mnid = ak.flatten(ak.Array(data_hits["sh.mnid"]))
    srn  = ak.flatten(ak.Array(data_hits["srn"]))
    plt.hist2d(np.asarray(srn), np.asarray(mnid), bins=[20, 300])
